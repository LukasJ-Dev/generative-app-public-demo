...
async function generateCode(query: string) {
  ...
  const { appCode } = await res.json();

  return appCode;
}

export default function SandboxedApp({ query }: { query: string }) {
  const [code, setCode] = useState<string | null>(null);

  console.log(!!query && !code);
  const { data, isLoading, error } = useQuery({
    queryKey: ["appCode", query],
    queryFn: () => generateCode(query),
    onSuccess: (data) => {
      setCode(data);
    },
    enabled: !!query && !code,
        ...
        <div className="h-screen w-full dark:bg-black bg-white  dark:bg-dot-white/[0.2] bg-dot-black/[0.2] relative flex items-center justify-center">
          ...
          <div className="flex flex-col items-center justify-center">
            ...
            <p className="text-white">
              This may take a few seconds upwards to a few minutes.
            </p>
          </div>
        </div>
      </div>
